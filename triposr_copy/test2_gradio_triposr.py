...
##########################STABLE DIFFUSION PART WITH A RANDOM IMAGE  #######################################
...
def stable_diffusion_t2i(prompt, outdir = "outputs/txt2img-samples", 
                         ...
                         n_iter = 3, H = 768, W = 768, device = "cuda", 
                         C = 4, f=8, n_samples = 3, n_rows = 0, scale = 9.0,
                         seed = 42, precision = "autocast", repeat = 3, 
    ...
    selected_image = choice(all_images)

    # Move selected image to a fixed path, overwriting previous selection
    final_image_path = os.path.join(outpath, "selected_output.png")
    shutil.move(selected_image, final_image_path)

    # Delete other images
    for image_path in all_images:
        if image_path != selected_image:
            os.remove(image_path)

    print(f"Your selected image is ready and waiting for you here: \n{final_image_path} \n"
          f" \nEnjoy.")
...
def process_image(input_image_path, output_dir, device='cuda:0', pretrained_model='stabilityai/TripoSR', chunk_size=8192,
    ...
    # Image processing
    print("removing background of the image")
    if remove_bg:
...
def generate_image_and_convert_to_3d(prompt):
    ...
    image_file = os.path.join("outputs/txt2img-samples", "selected_output.png")
...
# Definition of a function that handles image and 3D generation 
def generate_image_and_convert_to_3d(prompt):
    ...
    # Check if the image is correctly generated 
    image_file = os.path.join("outputs/txt2img-samples", "selected_output.png")
    if not os.path.exists(image_file):
        return "No image generated, please try again with another prompt.", None, None
    
    # Time after image generation
    ...
    # Time after 3D model generation
    model_time = time.time()

    # Calculate individual and total times
    image_gen_time = image_time - start_time
    model_gen_time = model_time - image_time
    total_time = model_time - start_time

    return image_file, model_3d_path, (image_gen_time, model_gen_time, total_time)

# Use gr.Blocks for the interface construction
with gr.Blocks() as demo:
    gr.Markdown("## Generate a 3D Object from a Prompt")
    gr.Markdown("Enter a prompt to generate an image, which will then be converted into a 3D model.")
    ...
    def update_outputs(prompt):
        image_path, model_3d_path, times = generate_image_and_convert_to_3d(prompt)
        if image_path and os.path.exists(image_path):
            with open(image_path, "rb") as file:
                img = Image.open(file)
                img.show()
...
# Launch the interface with the share option activated
demo.launch(share=True)
