...
import PIL
import sys
sys.path.append("/home/across-lab/miniconda3/envs/CRM2/lib/python3.9/site-packages")  # Replace with actual path
...
####################################################### Partie Othographic CCM --> Textured Mesh ############################################################################
#input model = 6 vues
#output = Mesj
#la on va charger le modèle
crm_path = hf_hub_download(repo_id="Zhengyi/CRM", filename="CRM.pth") #chemin du modèle
specs = json.load(open("configs/specs_objaverse_total.json"))#peu de specifications sur le modèle, les inputs et les outputs mais pas plus (architecture et hyperparamètres)
model = CRM(specs).to(args.device) #hyperparametres donnés au model puis model mis sur le gpu
model.load_state_dict(torch.load(crm_path, map_location = args.device), strict=False) # c'est enfin là qu'on load le modèle  depuis huggingface!!!
#il faudra executer la ligne ci dessous pour enregistrer le modèle en local :
#model = torch.load('model_entraine.ckpt')
# ou 
#torch.save(model.state_dict(), 'model_entraine.ckpt')
############################################################################################################################################################################

############################################# Partie SV Image -> MV Images #################################################################################################
