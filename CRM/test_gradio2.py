...
def prompt_to_image(prompt):
    ...
    out_path = "outputs/txt2img-samples/"
    grid_count = len(os.listdir(out_path)) - 2
    out_path = out_path + f"grid-{grid_count:04}.png"
    img = Image.open(out_path)
    return img

def image_to_3D(num_image):
    out_path = "outputs/txt2img-samples/samples"
    grid_count = len(os.listdir(out_path)) - 10 + num_image
...
with gr.Blocks() as demo:
    with gr.Row():
        with gr.Column():
            ...
            generate_image_btn = gr.Button(value="Générer plusieurs images")
            examples = gr.Examples(examples=["a rabbit with a yellow jacket", "a hamburger"], inputs=[prompt])

        with gr.Column():
            image_generee = gr.Image(label = "image 2D", image_mode = 'RGBA', sources = 'upload', type = 'pil', interactive = False)
    generate_image_btn.click(fn = prompt_to_image, inputs = [prompt], outputs = [image_generee])   
    
    with gr.Row():
        with gr.Column():
            num_image = gr.components.Slider(minimum = 1, maximum = 9, step =1, label = "Quelle image devrait servir à générer l'asset 3D ?")
            generate_asset_btn = gr.Button(value="asset 3D")
        with gr.Column():
            output_obj = gr.Model3D(interactive = False, label = "Output 3D asset")
    generate_asset_btn.click(fn = image_to_3D, inputs = [num_image], outputs = [output_obj])   
